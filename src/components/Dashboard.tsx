...
import { TransactionType, SmartFeedItem, InsightType, Account, AccountType, Bill, User } from '../types';
...
const TransferIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" /></svg>;
...
const SmartFeed: React.FC<{ finance: UseFinanceReturn; userPlan: 'free' | 'premium'; apiKey: string | null; openPremiumModal: () => void; }> = ({ finance, userPlan, apiKey, openPremiumModal }) => {
    const [feedItems, setFeedItems] = useState<SmartFeedItem[]>([]);
    ...
    useEffect(() => {
        if(userPlan === 'premium' && finance.transactions.length > 0) {
            generateFeed();
        } else {
            setIsLoading(false);
            if(userPlan === 'premium') setError("Add transactions to get insights.")
        }
    // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [finance.transactions, finance.budgets, finance.goals, apiKey, userPlan]);

    if (userPlan === 'free') {
        return (
             <Card className="h-full flex flex-col items-center justify-center text-center">
                 <h2 className="text-xl font-semibold">Smart Feed</h2>
                 <p className="text-gray-400 mt-2">Unlock AI-powered insights into your spending habits.</p>
                 <Button className="mt-4" onClick={openPremiumModal}>Upgrade to Premium</Button>
            </Card>
        )
    }
...
const Dashboard: React.FC<DashboardProps> = ({ finance, user, apiKey, setActiveView, openPremiumModal }) => {
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2">
                   <SmartFeed finance={finance} userPlan={user.plan} apiKey={apiKey} openPremiumModal={openPremiumModal} />
                </div>
                <div className="space-y-6">
                    <FinancialHealthCard finance={finance} setActiveView={setActiveView} />
                    <AIBriefingCard finance={finance} userPlan={user.plan} apiKey={apiKey} openPremiumModal={openPremiumModal}/>
                </div>
            </div>
