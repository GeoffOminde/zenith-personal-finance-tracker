...
// This safeguard is placed here to prevent the @google/genai SDK from
// attempting to register its own service workers from external origins.
if (typeof window !== 'undefined' && 'serviceWorker' in navigator) {
  ...
  (navigator.serviceWorker as any).register = (scriptURL: string | URL, options?: any) => {
    ...
    if (registrationUrl === allowedUrl) {
      // If the URL is the one we expect, proceed with the original function.
      return originalRegister(scriptURL, options);
    } else {
      // If not, block it, log a warning, and return a rejected promise.
      const warningMessage = `[Zenith Safeguard in geminiService] Blocked an attempt to register a service worker from an unauthorized URL: ${registrationUrl}`;
      console.warn(warningMessage);
      return Promise.reject(new Error('Registration of external service worker is not allowed.'));
    }
  };
}
// --- END PATCH ---
